import { Repository } from 'typeorm'
import { InjectRepository } from '@nestjs/typeorm'
import { addDocToElasticsearch, deleteAllDocByElasticsearch, indexElasticsearchExists } from 'src/utils/elasticsearch'
import { STOCK_OUT_ITEM_ELASTICSEARCH_INDEX } from 'src/constants/index.elasticsearch'
import { ConfigService } from '@nestjs/config'
import { Injectable, OnModuleInit } from '@nestjs/common'
import { StockOutItemEntity } from './stock-out-item.entity'

@Injectable()
export class StockOutItemRepo implements OnModuleInit {
  constructor(
    @InjectRepository(StockOutItemEntity)
    private readonly stockOutItemRepository: Repository<StockOutItemEntity>,
    private readonly configService: ConfigService
  ) {}

  async onModuleInit() {
    const isSync = this.configService.get('SYNC_MONGODB_TO_ELASTICSEARCH')
    if (isSync !== '1') {
      return
    }
    const result: StockOutItemEntity[] = await this.stockOutItemRepository.find()
    const indexExist = await indexElasticsearchExists(STOCK_OUT_ITEM_ELASTICSEARCH_INDEX)
    if (indexExist) {
      await deleteAllDocByElasticsearch(STOCK_OUT_ITEM_ELASTICSEARCH_INDEX)
    }
    for (const doc of result) {
      await addDocToElasticsearch(STOCK_OUT_ITEM_ELASTICSEARCH_INDEX, doc.stko_item_id.toString(), doc)
    }
  }
}
