...
export class TicketGuestRestaurantQuery {
  ...
  async getTicketRestaurant(
    ...
    try {
      ...
      const must: any = [
        {
          bool: {
            should: [{ match: { tkgr_res_id: account.account_restaurant_id } }]
          }
        }
      ]
      if (tkgr_priority) {
  ...
  async getTicketGuestIdUserGuest(id_user_guest: string): Promise<TicketGuestRestaurantEntity[]> {
    try {
      const indexExist = await indexElasticsearchExists(TICKET_GUEST_RESTAURANT_ELASTICSEARCH_INDEX)
      ...
      saveLogSystem({
        action: 'getTicketGuestIdUserGuest',
        class: 'TicketGuestRestaurantQuery',
        function: 'getTicketGuestIdUserGuest',
        message: error.message,
        time: new Date(),
        error: error,
        type: 'error'
